...
function list_build() {
    ...
    if [ "${target_list_name_slug}" == 'block-beacon' ] || [ "${target_list_name_slug}" == 'block-page-visibility' ] ||
     [ "${target_list_name_slug}" == 'block-svg' ] || [ "${target_list_name_slug}" == 'block-unsafe-eval' ] ||
     [ "${target_list_name_slug}" == 'block-webgl' ] || [ "${target_list_name_slug}" == 'block-webgpu' ] ||
     [ "${target_list_name_slug}" == 'block-webrtc' ] || [ "${target_list_name_slug}" == 'enable-gpc' ] ||
     [ "${target_list_name_slug}" == 'enable-dnt' ]; then
...
function build_list() {
    ...
    if [ "${list_name_slug}" != '3p' ] && [ "${list_name_slug}" != 'annoyances' ] && [ "${list_name_slug}" != 'trusted' ] &&
     [ "${list_hardened}" != 1 ]; then
        if [ "${BADBLOCK_BUILD_WC}" == 1 ]; then
