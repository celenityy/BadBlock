...
if [[ -z "${BADBLOCK_SET_ENVS+x}" ]]; then
  ...
  "${BADBLOCK_LN}" -sf "${BADBLOCK_SHA1SUM}"    "${BADBLOCK_PATH}/sha1sum"
  "${BADBLOCK_LN}" -sf "${BADBLOCK_SHA256SUM}"  "${BADBLOCK_PATH}/sha256sum"
  "${BADBLOCK_LN}" -sf "${BADBLOCK_SHA512SUM}"  "${BADBLOCK_PATH}/sha512sum"
