...
* ⭐️ [xRuffKez's Newly Registered Domains (NRDs)](https://github.com/hagezi/dns-blocklists?tab=readme-ov-file#new-newly-registered-domains-nrds-) - If you are unable to block NRDs on your DNS content blocking solution (like NextDNS), this is another vital list to add, as it heavily improves security by blocking newly registered domains, which are very commonly used for malicious purposes. We would recommend using the `14 days` variant.
